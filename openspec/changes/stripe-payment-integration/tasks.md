...
- [ ] 3.1 PR5 — Start: valid checkout. CREATE `src/modules/pedidos/{hooks/useCreatePaymentIntentMutation.ts,componentes/StripePaymentForm.tsx}`; MODIFY `src/modules/pedidos/pages/CheckoutPage.tsx`. Finish: module-local dynamic-`clientSecret` `Elements`, card-only lock; never mount Elements in `main.tsx`. Manual P01–P06: intent/rejection, 3DS/redirect, decline, duplicate submit, missing key; guidance/no sensitive values.
