...
export function PaymentOutcomePanel({ status, isLoading, hasTimedOut, isInaccessible, isRetrying, onRetry }: Props) {
  ...
  if (status.state === 'SUCCEEDED') return <Outcome icon={CheckCircle2} title="Pago confirmado" reference={`Pedido #${status.orderNumber}`} technicalId={status.orderId} message="Hemos confirmado el pago y creado tu pedido." tone="success" actionLabel="Ver mi pedido" to={`/pedidos?orderId=${encodeURIComponent(status.orderId)}`} />
...
function Outcome({ icon: Icon, title, reference, technicalId, message, tone = 'default', actionLabel, onAction, isActionPending = false, to }: { icon: typeof Clock3; title: string; reference?: string; technicalId?: string; message: string; tone?: 'default' | 'success' | 'error'; actionLabel?: string; onAction?: () => void; isActionPending?: boolean; to?: string }) {
  ...
  return <section aria-live="polite" className="border border-[color-mix(in_srgb,var(--color-outline-variant)_80%,transparent)] bg-[var(--color-surface-container-lowest)] p-7 shadow-sm md:p-10"><span className={`mb-6 flex size-12 items-center justify-center rounded-full ${iconClass}`}><Icon size={24} strokeWidth={1.8} /></span><h1 className="text-display-md text-[var(--color-on-surface)]">{title}</h1>{reference ? <p className="text-headline-md mt-3 text-[var(--color-primary)]">{reference}</p> : null}{technicalId ? <p className="text-label-sm mt-1 max-w-xl break-all font-mono text-[var(--color-outline)]">ID técnico: {technicalId}</p> : null}<p className="text-body-lg mt-4 max-w-xl text-[var(--color-on-surface-variant)]">{message}</p>{onAction ? <button type="button" disabled={isActionPending} onClick={onAction} className="text-label-md mt-8 inline-flex items-center gap-2 bg-[var(--color-primary)] px-5 py-3 text-[var(--color-on-primary)] transition-colors hover:bg-[var(--color-primary-container)] disabled:cursor-not-allowed disabled:opacity-60"><RotateCcw size={16} strokeWidth={1.8} />{actionLabel}</button> : null}{to ? <Link to={to} className="text-label-md mt-8 inline-flex items-center gap-2 bg-[var(--color-primary)] px-5 py-3 !text-[var(--color-on-primary)] transition-colors hover:bg-[var(--color-primary-container)] hover:!text-[var(--color-on-primary)]">{actionLabel}</Link> : null}</section>
}
