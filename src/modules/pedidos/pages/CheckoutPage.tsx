...
import { StripePaymentForm } from '../componentes/StripePaymentForm'
import { useCreatePaymentIntentMutation } from '../hooks/useCreatePaymentIntentMutation'
...
export function CheckoutPage() {
  ...
  function discardPaymentIntent(message: string) {
  ...
  useEffect(() => {
    function discardRestoredIntent(event: PageTransitionEvent) {
      if (event.persisted) discardPaymentIntent('La sesión de pago anterior ya no está disponible. Revisa la entrega y prepara un nuevo pago.')
    }

    window.addEventListener('pageshow', discardRestoredIntent)
    return () => window.removeEventListener('pageshow', discardRestoredIntent)
  }, [])
  ...
  async function continueToPayment() {
    ...
    try {
      const clientSecret = await createPaymentIntentMutation.mutateAsync({
        deliverySelections: validSelections,
        ...(requiresShippingAddress ? { addressId: activeSelectedAddressId! } : {}),
      })
      setClientSecret(clientSecret)
      setValidationError(null)
      setCurrentStep(2)
    } catch (error) {
      setValidationError(resolveErrorMessage(error))
    }
  }
