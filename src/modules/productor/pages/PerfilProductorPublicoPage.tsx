...
const catalogPreview = [
  ...
  },
  {
    ...
    detail: '400g - Elaboración Manual',
    price: '12,90 €',
    badge: 'Orgánico',
    image:
        ...
        <section className="mt-16">
          ...
          <div className="grid grid-cols-1 gap-[var(--space-gutter)] sm:grid-cols-2 lg:grid-cols-4">
            {catalogPreview.map((product) => (
              <article key={product.name} className="flex flex-col border border-[color-mix(in_srgb,var(--color-outline)_16%,transparent)] bg-white p-4 transition-transform duration-300 hover:-translate-y-1">
                <div className="relative mb-4 aspect-square overflow-hidden bg-[var(--color-surface-container-low)]">
                  <img src={product.image} alt={product.name} className="size-full object-cover transition-transform duration-700 hover:scale-105" />
                  {product.badge ? (
                    <span className="text-label-sm absolute top-2 left-2 bg-[var(--color-surface-container-low)] px-2 py-1 text-[10px] uppercase tracking-[0.18em] text-[var(--color-secondary)]">
                      {product.badge}
                    </span>
                  ) : null}
                </div>
                <h3 className="text-headline-md mb-1 text-[18px] leading-tight text-[var(--color-primary)]">{product.name}</h3>
                <p className="text-label-sm mb-4 text-[var(--color-secondary)]">{product.detail}</p>
                <p className="text-body-md mt-auto font-bold text-[var(--color-on-surface)]">{product.price}</p>
              </article>
            ))}
          </div>
        </section>
