...
import { AlertTriangle, ArrowRight, CheckCircle2, Info, Package, ShoppingBag, X } from 'lucide-react'
...
export function UserActionModals({ modal, onClose }: UserActionModalsProps) {
  if (modal?.type === 'detail') {
    return <UserDetailDrawer user={modal.user} onClose={onClose} />
  }

  if (modal?.type === 'activate') {
    return <ActivateUserModal onClose={onClose} />
  }

  if (modal?.type === 'deactivate') {
    return <DeactivateUserModal onClose={onClose} />
  }

  if (modal?.type === 'delete') {
    return <DeleteUserModal onClose={onClose} />
  }
...
function UserDetailDrawer({ user, onClose }: UserModalProps) {
  return (
    <div className="fixed inset-0 z-[100] flex justify-end" role="dialog" aria-modal="true">
      ...
      <aside className="relative flex h-full w-full max-w-md flex-col border-l border-[color-mix(in_srgb,var(--color-secondary)_35%,transparent)] bg-[var(--color-background)] shadow-[0_20px_60px_-20px_rgba(28,27,27,0.35)]">
        ...
        <div className="flex-1 space-y-12 overflow-y-auto p-8">
          {user.coverImage && (
            <img src={user.coverImage} alt={`Portada de ${user.name}`} className="h-48 w-full object-cover" />
          )}
          ...
          <DrawerSection title="Resumen de actividad">
            <div className="grid grid-cols-2 gap-4">
              <MetricCard icon={Package} value={String(user.productsPublished ?? 12)} label="Productos publicados" />
              <MetricCard icon={ShoppingBag} value={String(user.totalOrders ?? 38)} label="Pedidos totales" />
            </div>
          </DrawerSection>
        </div>
        ...
        <footer className="shrink-0 border-t border-[color-mix(in_srgb,var(--color-secondary)_25%,transparent)] bg-[var(--color-background)] p-8">
          <div className="flex flex-col gap-4">
            <button
              type="button"
              className="text-label-md w-full bg-[var(--color-primary-container)] px-6 py-4 text-[var(--color-on-primary)] transition-colors hover:bg-[var(--color-primary)]"
            >
              Moderar contenido
            </button>
            <button
            ...
            >
              {user.status === 'Activa' ? 'Desactivar' : 'Activar'}
            </button>
            <button
              type="button"
...
function ActivateUserModal({ onClose }: { onClose: () => void }) {
  return (
    <CenteredOverlay>
      <section className="relative z-10 flex w-full max-w-md flex-col items-center border border-[color-mix(in_srgb,var(--color-secondary)_45%,transparent)] bg-[var(--color-background)] p-10 text-center shadow-[0_20px_40px_-15px_rgba(28,27,27,0.15)]">
        ...
        <h2 className="text-headline-lg mb-4 text-[var(--color-on-surface)]">Activar cuenta</h2>
        <p className="text-body-md mx-auto mb-10 max-w-xs leading-relaxed text-[var(--color-outline)]">
          ¿Está seguro de que desea activar esta cuenta? Se restaurará el acceso del usuario y se le enviará una
        ...
        <div className="flex w-full flex-col gap-4">
          ...
          >
            Activar
            <ArrowRight size={18} strokeWidth={1.8} className="transition-transform group-hover:translate-x-1" />
          </button>
          <button
            type="button"
            onClick={onClose}
...
function DeactivateUserModal({ onClose }: { onClose: () => void }) {
  return (
    <CenteredOverlay>
...
function DeleteUserModal({ onClose }: { onClose: () => void }) {
  return (
    <CenteredOverlay>
        ...
        <div className="flex flex-col gap-8 p-8 text-center md:p-10">
          ...
          <div className="flex flex-col gap-6">
            <p className="text-body-lg text-[var(--color-on-surface-variant)]">
              ¿Está seguro de que desea eliminar esta cuenta? Esta acción dará de baja la cuenta conforme a las
              políticas de la plataforma y{' '}
...
function CenteredOverlay({ children }: CenteredOverlayProps) {
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      <div className="absolute inset-0 bg-[color-mix(in_srgb,var(--color-on-surface)_24%,transparent)] backdrop-blur-[2px]" />
      {children}
    </div>
  )
}
...
function RoleBadge({ role }: { role: AdminUser['role'] }) {
  ...
  return <span className={`text-label-sm rounded-[var(--radius-sm)] px-3 py-1 uppercase tracking-wider ${className}`}>{role}</span>
}
